...
// Get POST Values
$state_id    = get_safe_value($con,@$_POST['state_id']);
$title    = get_safe_value($con,@$_POST['title']);

// Insert new entry Query
if(@$_GET['mode']=="addnew"){
    $sql = "INSERT INTO `$tblname`(`status`,`state_id`,`name`) 
    VALUES ('1','$state_id', '$title')";
...
$img_required ="";
  $sql="UPDATE `$tblname` SET `state_id`='$state_id',`name`='$title' WHERE ID =$id";
  if (!mysqli_query($con,$sql)){die('Error: ' . mysqli_error($con)); }
...
$sql = "SELECT c.*,s.name as state_name FROM `$tblname` c
            ...
            <?php } ?>
              <div class="box-body">
                ...
                <div class="form-group col-md-4">
                  <label for="state_id">State <span class="text-red">*</span></label>
                  <select class="form-control" name="state_id" required id="state_id">
                    <option class="text-blue bold">Select State</option>
                    <?php  
                      $res = mysqli_query($con,"SELECT ID,name FROM `state` ORDER BY ID ASC");
                ...
                <div class="form-group col-md-4">
                  ...
                  <input type="text" class="form-control" name="title" value="<?= @$get_title; ?>" autocomplete="off" placeholder="City Name" required>
                </div>
          ...
          <table id="example1_samy" class="table table-bordered table-striped table-hover">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td valign="middle">
                  <span class="text-blue bold font-15">State Name: </span><?php echo $row['state_name']; ?><br>
                  <span class="text-red bold font-15">Title: </span><?php echo $row['name']; ?><br>
